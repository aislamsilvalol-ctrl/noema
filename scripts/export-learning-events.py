...
from noema.services.learning_export import export_events, to_jsonl
...
async def main() -> int:
    ...
    until = (
        datetime.fromisoformat(args.until).replace(tzinfo=UTC) if args.until else None
    )
    n = 0
    async with get_sessionmaker()() as db:
        # One append per event between database round trips; buffering the
        # whole export in memory would only trade a crash for a bigger loss.
        with Path(args.out).open("w") as f:  # noqa: ASYNC230
            async for event in export_events(
                db, secret=secret, since=since, until=until
            ):
