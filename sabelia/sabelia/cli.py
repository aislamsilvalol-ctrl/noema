"""``sabelia`` command line: simulate, benchmark, train, evaluate, compare, serve.

    sabelia simulate --students 300 --out data/synthetic.jsonl
    sabelia benchmark --dataset configs/datasets/synthetic.yaml --out runs/
    sabelia train --dataset configs/datasets/synthetic.yaml \\
        --config configs/train/sabelia.yaml --out runs/
    sabelia compare --runs runs/
    sabelia serve --model runs/models/sabelia/<version>
...
from sabelia.data.adapters import load_dataset, write_jsonl
from sabelia.evaluation.metrics import reliability_table, summarize
...
def cmd_benchmark(a: argparse.Namespace) -> int:
    from sabelia.benchmarks import run_benchmark  # noqa: PLC0415
...
def cmd_train(a: argparse.Namespace) -> int:
    from sabelia.training.trainer import TrainConfig, save_checkpoint, train  # noqa: PLC0415
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="sabelia", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
