...
Sabelia is research code held to engineering standards. Three rules:

1. **Measure before you add.** A new model or feature comes with a
   benchmark row (`sabelia benchmark`) and, if it is an ingredient, an
...
.venv/bin/pytest
.venv/bin/ruff check sabelia tests && .venv/bin/ruff format --check sabelia tests
```
