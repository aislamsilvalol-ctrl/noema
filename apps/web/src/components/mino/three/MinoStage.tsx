...
function Rig({ pose, blink, motion, framing }: { pose: Pose; blink: number; motion: boolean; framing: Framing }) {
  ...
  const base = useRef<{ eyeR: THREE.Vector3; eyeL: THREE.Vector3; mouth: THREE.Vector3 } | null>(null);
  ...
  return (
    <>
      ...
      <Environment resolution={64} frames={1}>
        ...
        <Lightformer intensity={0.5} color="#e9dccb" position={[0, -2, 0]} rotation={[Math.PI / 2, 0, 0]} scale={[5, 5, 1]} />
      </Environment>
      <ContactShadows position={[0, framing === 'bust' ? -0.72 : -0.56, 0]} opacity={0.42} scale={2.4} blur={2.2} far={1.2} resolution={256} frames={1} />
    </>
  );
}
