...
function Page() {
  ...
  const bank: SubjectBank = bankFor(asked ?? copy.hero.subjects[0] ?? '', locale);
  ...
  useEffect(() => {
    ...
    if (typeof window.matchMedia === 'function' && window.matchMedia('(prefers-reduced-motion: reduce)').matches)
      return;
    const timer = window.setInterval(() => setRotating((i) => (i + 1) % copy.hero.subjects.length), 2600);
  ...
  const stepAt = (i: number) => steps[i] ?? { n: String(i + 1), title: '', body: '' };
    ...
    <main className="min-h-screen">
        ...
        <div>
          <h1 className="font-display text-5xl text-ink-900 md:text-6xl">
            ...
            <span className="mt-2 block text-signal" aria-live="off">
              <span key={rotatingSubject} className="inline-block animate-fade-up">
                {rotatingSubject}
              </span>
            </span>
          </h1>
          ...
          <form onSubmit={ask} className="mt-10">
            <label htmlFor="ask" className="block font-display text-xl text-ink-900">
              {copy.hero.label}
            </label>
            ...
            {!asked && <p className="mt-3 text-sm text-ink-500">{copy.hero.note}</p>}
          </form>
              ...
              {status !== 'streaming' && (
                <div className="mt-4 flex flex-wrap items-center gap-4">
                  <p className="text-xs text-ink-400">
                    {status === 'live' ? copy.hero.liveNote : copy.hero.sampleNote}
                  </p>
                  <button
                    type="button"
      ...
      <section className="border-t border-line">
        <div className="mx-auto max-w-6xl px-6 pt-20 md:pt-28">
          ...
          <h2 className="mt-4 max-w-4xl font-display text-3xl text-ink-900 md:text-4xl">{copy.engine.title}</h2>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-6 pb-8 pt-12 md:pt-16">
          <Step id="step-1" register={register} step={stepAt(0)} state={active}>
            <Dialogue
              lines={[
                {
                  who: t.chat.you,
                  text: asked ? `${copy.engine.askExample.replace(/Freud/i, asked)}` : copy.engine.askExample,
                },
              ]}
            />
          </Step>
            ...
            <ol data-landing-path className="max-w-md">
              {bank.path.map((module, index) => (
                ...
                >
                  ...
                  <span className={`font-mono text-xs ${index === 0 ? 'text-signal' : 'text-ink-400'}`}>
          ...
          <Step id="step-4" register={register} step={stepAt(3)} state={active}>
            <div className="max-w-md">
              ...
              <Markdown text={lessonText} className="mt-3 font-serif text-md text-ink-900" />
            </div>
          </Step>

          <Step id="step-5" register={register} step={stepAt(4)} state={active}>
            <div className="max-w-md">
              <p className="font-display text-lg text-ink-900">{bank.question}</p>
                      ...
                      >
                        <span
                          ...
                          className={`mt-1.5 inline-block h-2 w-2 shrink-0 rounded-full border transition-colors duration-fast ${
                            chosen ? 'border-current bg-current' : 'border-ink-300'
                          }`}
                        />
                        {option}
                      </button>
              ...
              {answer !== null && !answered && (
                <div className="mt-4 flex items-center gap-3 border-t border-line pt-4">
                  <span className="text-sm text-ink-600">{copy.engine.confident}</span>
              ...
              {answered && !correct && <p className="mt-2 text-sm text-ink-700">{bank.correction}</p>}
            </div>
          </Step>
              ...
              return (
                <div className="max-w-md">
                  <p className="text-xs text-ink-400">
                    ...
                    {moved && <span className="text-signal"> · {copy.engine.masteryMoved}</span>}
                  </p>
                  <ul className="mt-3">
                    {rows.map((row) => (
                        ...
                        <div className="mt-2 h-px w-full bg-line">
                          <div
                            className={`h-px transition-[width] duration-slow ease-noema ${
                              row.value < 40 ? 'bg-critical' : row.value < 60 ? 'bg-ink-500' : 'bg-positive'
                            }`}
                  ...
                  <p className="mt-3 text-xs text-ink-400">{copy.engine.masteryHint}</p>
                </div>
              );
            })()}
          </Step>
          ...
          <Step id="step-8" register={register} step={stepAt(7)} state={active}>
            <div className="max-w-md">
              <Speaker>Mino</Speaker>
              <p className="mt-2 font-serif text-md text-ink-900">{copy.engine.returnWhen(REMEMBERED_DAYS)}</p>
              <button
                type="button"
              ...
              >
                ...
                >
                  <div className="col-start-1 row-start-1 rounded-lg border border-line bg-raised p-6 [backface-visibility:hidden]">
                    ...
                    <p className="mt-6 text-xs text-ink-400">{copy.engine.returnTap}</p>
                  </div>
                  <div className="col-start-1 row-start-1 rounded-lg border border-signal bg-raised p-6 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                    ...
                    <p className="mt-5 text-xs text-ink-500">{copy.engine.returnNext(NEXT_REVIEW_DAYS)}</p>
                  </div>
                </div>
              </button>
            </div>
      ...
      {/* ── versus ──────────────────────────────────────────────────────── */}
      <section ref={register('versus')} data-section="versus" className="border-t border-line">
        <div className="mx-auto max-w-6xl px-6 py-20 md:py-28">
          <h2 className="max-w-3xl font-display text-3xl text-ink-900 md:text-4xl">{copy.versus.title}</h2>
          <p className="mt-4 max-w-reading text-md text-ink-600">{copy.versus.body}</p>
          <dl className="mt-12 max-w-4xl">
            {copy.versus.pairs.map(([them, us]) => (
              <div key={us} className="grid gap-2 border-t border-line py-5 md:grid-cols-2 md:gap-10">
      ...
      {/* ── rhythm ──────────────────────────────────────────────────────── */}
      <section ref={register('rhythm')} data-section="rhythm" className="border-t border-line">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] md:py-28">
          <div className="max-w-reading">
            ...
            <h2 className="mt-4 font-display text-3xl text-ink-900">{copy.rhythm.title}</h2>
            <p className="mt-4 text-md text-ink-600">{copy.rhythm.body}</p>
          </div>
          <div data-landing-mode={demoMode} className="max-w-md md:justify-self-end">
            <div className="flex gap-6 border-b border-line" role="tablist" aria-label={copy.rhythm.title}>
              {(['normal', 'focus'] as const).map((m) => (
                <button
                  ...
                  className={`-mb-px border-b-2 py-2 text-sm transition-colors duration-fast ${
                    demoMode === m ? 'border-signal text-ink-900' : 'border-transparent text-ink-500 hover:text-ink-900'
                  }`}
                >
              ...
              (() => {
                ...
                return (
                  <ol className="mt-6 space-y-5">
                    ...
                    <li>
                      <p className="text-xs text-signal">{copy.rhythm.concept}</p>
                    ...
                    <li>
                      <p className="text-xs text-signal">{copy.rhythm.interaction}</p>
                      <p className="mt-1 text-sm text-ink-900">{bank.question}</p>
                    ...
                    <li>
                      <p className="text-xs text-signal">{copy.rhythm.recall}</p>
                      <p className="mt-1 font-serif text-sm text-ink-900">{bank.card.front}</p>
                    </li>
                  </ol>
                );
              })()
      ...
      {/* ── close ───────────────────────────────────────────────────────── */}
      <section ref={register('close')} data-section="close" className="border-t border-line">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-24 md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] md:items-center md:py-32">
          ...
          <div>
            <h2 className="font-display text-5xl text-ink-900 md:text-6xl">{copy.close.title}</h2>
            <p className="mt-6 max-w-reading text-md text-ink-600">{copy.close.body}</p>
    ...
    <ol className="max-w-md">
      {lines.map((line, index) => (
        <li key={`${index}-${line.text}`} className="border-t border-line py-3">
          ...
          <p className={`mt-1 text-md ${line.who === 'Mino' ? 'text-ink-900' : 'font-serif italic text-ink-700'}`}>
            {line.text}
          </p>
        </li>
      ))}
